    ...
    <Box ref={dropRef} cursor={'all-scroll'}>
      {isDragging ? (
        ...
        <Flex ref={composedRefs} data-handler-id={handlerId}>
          <IconButton tag="div" role="button" tabIndex={0} aria-label="Drag" borderWidth={0} onKeyDown={handleKeyDown}>
            <Drag />
          </IconButton>
          {'My item'}
        </Flex>
...
const MyComponent = ({ onDropItem }) => {
  const [{ handlerId, handleKeyDown, isOverDropTarget, direction }, myRef, dropRef, dragRef] = useDragAndDrop(true, {
    type: 'my-type',
    index,
    onDropItem,
  });
...
const MyComponent = ({ onMoveItem }) => {
  const [{ handlerId, isDragging, handleKeyDown }, myRef, dropRef, dragRef, dragPreviewRef] = useDragAndDrop(true, {
    type: 'my-type',
    index,
    onMoveItem,
  });
