...
*Entities that define the physics/rules of the world.*
...
*Standalone game elements that use Atoms.*
...
*Complex structures that aggregate Molecules.*
...
*The Blueprints (not the instances).*
...
*Goal: A headless, fully tested engine capable of generating valid entities.*
...
*Goal: Verify the engine works via terminal before building complex UI.*
...
*Goal: Deep integration into the Strapi workflow.*
...
*Goal: 100% Coverage and Stability.*
...
*Goal: Generate a "Starter Set" of Level 1 Entities for every Race/Class combination.*
...
1.  **State Storage**: For the "Chatbot" history, should we store conversation logs in the DB or keep them ephemeral (session-only)?
    *Assumption: Ephemeral for now, maybe local storage.*
2.  **Cost Control**: Do we need rate limiting or token counting for Gemini 3 usage?
    *Assumption: User provides API key, basic logging suffices.*
