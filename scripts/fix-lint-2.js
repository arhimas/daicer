...
replaceFile('src/features/genesis-core/audit-service.ts', (cnt) => cnt.replace(/: any/g, ': unknown'));

// 3. Unused variables in schemas
replaceFile('src/features/genesis-core/data/schemas/monster-blueprint-schema.ts', (cnt) =>
  cnt
    .replace(/APIReferenceSchema(,| )/g, '') // Simplified just to comment or remove if needed, but let's try basic AST cleanup or disable next line.
...
replaceFile('src/features/genesis-core/data/schemas/monster-schema.ts', (cnt) =>
  cnt
    .replace(/DamageSchema(,| )/g, '') // remove from imports
...
replaceFile('src/features/genesis-core/json-schema-builder.ts', (cnt) =>
  cnt.replace(/: any/g, ': unknown').replace(/@ts-ignore/g, '@ts-expect-error')
);

// 6. llm-bridge.ts any
replaceFile('src/features/genesis-core/llm-bridge.ts', (cnt) =>
  cnt.replace(/: any/g, ': unknown').replace(/<any>/g, '<unknown>')
);
...
replaceFile('src/features/genesis-core/mappers/__tests__/spell-mapper.test.ts', (cnt) =>
  cnt
    .replace(/'\.\.\/spell-mapper'/g, "'@/features/genesis-core/mappers/spell-mapper'")
...
replaceFile('src/features/genesis-core/mappers/action-mapper.ts', (cnt) =>
  cnt
    .replace(/'\.\.\/data\/schemas\/common-schemas'/g, "'@/features/genesis-core/data/schemas/common-schemas'")
    .replace(/DCMethodSchema, /g, '')
);

// 9. schema-loader.ts any
replaceFile('src/features/genesis-core/schema-loader.ts', (cnt) => cnt.replace(/: any/g, ': unknown'));
...
replaceFile('src/plugins/map-explorer/server/src/services/__tests__/gemini-service.test.ts', (cnt) =>
  cnt
    .replace(/\/\/ const service =/g, '/* const service = */') // ensuring it's fully commented or deleted
