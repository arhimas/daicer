...
async function main() {
  ...
  fs.writeFileSync(
    path.join(schemaDir, '_entity-list.json'),
    JSON.stringify(
      contentTypes.map((c) => c.uid),
      null,
      2
    )
  );
  ...
  console.log(
    `Done. Generated ${contentTypes.length} Entity files and ${componentList.length} Component files in schema/`
  );
}
