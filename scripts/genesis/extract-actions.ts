...
const ActionExtractionSchema = z.object({
  actions: z.array(
    z.object({
      name: z
        .string()
        .describe("The name of the action, ability, or trait (e.g., 'Bite', 'Multiattack', 'Amphibious')"),
      ...
      toHit: z
        ...
        .optional()
        .nullable()
        .describe('The attack bonus to hit (e.g., +5 -> 5). Null if not an attack.'),
      range_config: z
        .object({
...
${fullText}
      ...
      try {
        ...
        for (const action of result.actions) {
          const actionSlug = `${monsterSlug}-${action.name
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/(^-|-$)/g, '')}`;
