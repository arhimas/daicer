  ...
  .action(async (options) => {
    try {
      ...
      const {
        ...
        MagicSchoolMapper,
        BackgroundMapper,
      } = require('../../src/features/genesis-core/mappers/primitive-mappers');
      ...
      try {
        ...
        for (const dt of damageTypes) {
          ...
          if (await entityExists('damage-type', dt.index)) {
            console.log(`⏩ Skipping ${dt.name}`);
            count++;
            continue;
          }
        ...
        console.log('Skipping Damage Types', e.message);
      }
      ...
      try {
        ...
        for (const cond of conditions) {
          ...
          if (await entityExists('status-effect', cond.index)) {
            console.log(`⏩ Skipping ${cond.name}`);
            count++;
            continue;
          }
        ...
        console.log('Skipping Conditions', e.message);
      }
      ...
      try {
        ...
        for (const sch of schools) {
          ...
          if (await entityExists('magic-school', sch.index)) {
            console.log(`⏩ Skipping ${sch.name}`);
            count++;
            continue;
          }
        ...
        console.log('Skipping Magic Schools', e.message);
      }
      ...
      try {
        ...
        for (const bg of backgrounds) {
          ...
          if (await entityExists('background', bg.index)) {
            console.log(`⏩ Skipping ${bg.name}`);
            count++;
            continue;
          }
        ...
        console.log('Skipping Backgrounds', e.message);
      }
      ...
      try {
        ...
        const weaponPropertyMapper =
          new (require('../../src/features/genesis-core/mappers/primitive-mappers').WeaponPropertyMapper)();

        for (const wp of weaponProperties) {
          if (count >= limit) break;
          console.log(`Generating Weapon Property: ${wp.name}`);
          if (await entityExists('weapon-property', wp.index)) {
            console.log(`⏩ Skipping ${wp.name}`);
            count++;
            continue;
          }
        ...
        console.log('Skipping Weapon Properties', e.message);
      }
      ...
      try {
        ...
        for (const trait of traits) {
          ...
          if (await entityExists('trait', trait.index)) {
            console.log(`⏩ Skipping ${trait.name}`);
            count++;
            continue;
          }
        ...
        console.log('Skipping Traits', e.message);
      }
      ...
      try {
        ...
        for (const feat of features) {
          ...
          if (await entityExists('feature', feat.index)) {
            console.log(`⏩ Skipping ${feat.name}`);
            count++;
            continue;
          }
        ...
        console.log('Skipping Features', e.message);
      }
      ...
      for (const spell of spells) {
        ...
        if (await entityExists('spell', spell.index)) {
          console.log(`⏩ Skipping ${spell.name}`);
          count++;
          continue;
        }
      ...
      for (const item of items) {
        ...
        if (await entityExists('item', item.index)) {
      ...
      try {
        ...
        for (const cls of classes) {
          ...
          if (await entityExists('class', cls.index)) {
            console.log(`⏩ Skipping ${cls.name}`);
            count++;
            continue;
          }
        ...
        console.log('Skipping classes (loader mismatch or file missing)', e.message);
      }
      ...
      try {
        ...
        for (const race of racesToIngest) {
          ...
          if (await entityExists('race', race.index)) {
        ...
        console.log('Skipping races', e.message);
      }
      ...
      try {
        ...
        for (const monster of monsters) {
          ...
          const jsonSchema = await schemaBuilder.build(
            monsterMapper.getSchemaIdentifier ? monsterMapper.getSchemaIdentifier() : request.uid
          );
        ...
        console.log('Skipping monsters', e.message);
      }
