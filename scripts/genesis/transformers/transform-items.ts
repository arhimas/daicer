...
const EquipmentDataSchema = z
  .object({
    ...
    properties: z.preprocess(
      (val: any) => (Array.isArray(val) ? val.map(toSlug) : val ? [toSlug(val)] : val),
      ...
      .array(
        z.object({
          ...
          damage_type: z.preprocess(
            (val) => normalizeEnum(val, CasingEnums.damage_type_enum),
            z
              .enum(CasingEnums.damage_type_enum as any)
              .optional()
              .nullable()
          ),
...
const ItemSchema = z
  .object({
    ...
    type: z.preprocess(
      (val) => normalizeEnum(val, CasingEnums.item_type, 'loot'),
      z.enum(CasingEnums.item_type as any)
    ),
