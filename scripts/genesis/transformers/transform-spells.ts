...
const ConditionInstanceSchema = z.object({
  condition: z.preprocess(
    (val) => normalizeEnum(val, CasingEnums.condition, 'Special'),
    z.enum(CasingEnums.condition as any)
  ),
...
const ScalingConfigSchema = z.object({
  ...
  type: z.preprocess(
    (val) => normalizeEnum(val, CasingEnums.scaling_type, 'Dice'),
    z.enum(CasingEnums.scaling_type as any)
  ),
  method: z.preprocess(
    (val) => normalizeEnum(val, CasingEnums.scaling_method, 'Per Slot Level'),
    z.enum(CasingEnums.scaling_method as any)
  ),
...
const SpellSchema = z
  .object({
    ...
    school: z.preprocess(
      (val) => normalizeEnum(val, CasingEnums.school),
      z
        .enum(CasingEnums.school as any)
        .optional()
        .nullable()
    ),
    casting_config: z
      .object({
        ...
        time_unit: z.preprocess(
          (val) => normalizeEnum(val, CasingEnums.time_unit, 'Action'),
          z.enum(CasingEnums.time_unit as any)
        ),
    ...
    range_config: z
      .object({
        type: z.preprocess(
          (val) => normalizeEnum(val, CasingEnums.range_type, 'Ranged (Feet)'),
          z.enum(CasingEnums.range_type as any)
        ),
        distance: z.number().optional().nullable(),
        aoe_shape: z.preprocess(
          (val) => normalizeEnum(val, CasingEnums.aoe_shape),
          z
            .enum(CasingEnums.aoe_shape as any)
            .optional()
            .nullable()
        ),
    ...
    duration_config: z
      .object({
        type: z.preprocess(
          (val) => normalizeEnum(val, CasingEnums.duration_type, 'Instantaneous'),
          z.enum(CasingEnums.duration_type as any)
        ),
        value: z.number().optional().nullable(),
        unit: z.preprocess(
          (val) => normalizeEnum(val, CasingEnums.duration_unit),
          z
            .enum(CasingEnums.duration_unit as any)
            .optional()
            .nullable()
        ),
    ...
    mechanics_config: z
      .object({
        action_type: z.preprocess(
          (val) => normalizeEnum(val, CasingEnums.action_type, 'None'),
          z.enum(CasingEnums.action_type as any)
        ),
        save_effect: z.preprocess(
          (val) => normalizeEnum(val, CasingEnums.save_effect),
          z
            .enum(CasingEnums.save_effect as any)
            .optional()
            .nullable()
        ),
      })
    ...
    damage_instances: z
      .array(
        z.object({
          effect_type: z.preprocess(
            (val) => normalizeEnum(val, CasingEnums.effect_type, 'Damage'),
            z.enum(CasingEnums.effect_type as any)
          ),
          damage_type: z.preprocess(
            (val) => normalizeEnum(val, CasingEnums.damage_type),
            z
              .enum(CasingEnums.damage_type as any)
              .optional()
              .nullable()
          ),
          ...
          timing: z.preprocess(
            (val) => normalizeEnum(val, CasingEnums.timing, 'Instant'),
            z.enum(CasingEnums.timing as any)
          ),
        })
      )
      .optional()
