...
replace('src/features/genesis-core/dry-run-service.ts', cnt => 
  cnt.replace(/catch \(_e\)/g, 'catch (e)')
...
// 3. action-mapper.ts
replace('src/features/genesis-core/mappers/action-mapper.ts', cnt => 
  cnt.replace(/Record<string, unknown>/g, 'any')
);

// 4. item-mapper.ts
replace('src/features/genesis-core/mappers/item-mapper.ts', cnt => 
  cnt.replace(/Record<string, unknown>/g, 'any')
);
