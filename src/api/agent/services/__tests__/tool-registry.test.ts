...
vi.mock('@daicer/engine/world', () => ({
  WorldAtlas: class {
    constructor() {}
    getRegion() {
      return { name: 'Region', biome: 'Forest', wealth: 0.8 };
    }
    getStructure() {
      return { type: 'Village', name: 'TestVille' };
    }
  },
}));
...
const mockStrapi: any = {
  service: vi.fn((uid) => {
    ...
    if (uid === 'api::voxel-engine.voxel-engine')
      return { getChunk: vi.fn(() => ({ width: 32, height: 32, data: [] })) };
    return {};
  }),
  documents: vi.fn(() => ({
    findOne: mockFindOne,
...
describe('Tool Registry', () => {
  ...
  it('get_available_actions should dehydrate entity', async () => {
    mockFindOne.mockResolvedValueOnce({
      documentId: 'e1',
      inventory: [{ isEquipped: true, item: { type: 'weapon', equipment_data: {} } }],
      stats: { strength: 10 },
      level: 1,
    });
  ...
  it('search_monsters should find monsters', async () => {
    mockFindMany.mockResolvedValueOnce([{ name: 'Goblin', hp: 7, ac: 15, stats: {} }]);
      ...
      expect.objectContaining({
        data: expect.objectContaining({
          entropyState: expect.objectContaining({
            conditions: expect.arrayContaining([expect.objectContaining({ key: 'Gravity', currentValue: 'Low' })]),
          }),
        }),
      })
    );
  ...
  describe('Search Tools', () => {
    ...
    it('search_classes should find classes', async () => {
      mockFindMany.mockResolvedValueOnce([{ name: 'Wizard', hit_die: 'd6', proficiencies: [{ name: 'Daggers' }] }]);
  ...
  describe('Environment Tools', () => {
    ...
    it('get_weather should return weather', async () => {
      mockFindOne.mockResolvedValueOnce({
        entropyState: { conditions: [{ key: 'Local Weather', currentValue: 'Storm' }] },
      });
  ...
  describe('Action Wrapper Tools', () => {
    ...
    it('interact_object should dispatch command', async () => {
      await registry.execute(
        'interact_object',
        'room-1',
        { actorId: 'a1', targetId: 't1', interactionType: 'touch' },
        {}
      );
      expect(mockDispatch).toHaveBeenCalledWith(
        'room-1',
        expect.arrayContaining([expect.objectContaining({ type: 'INTERACT' })])
      );
    });
    ...
    it('modify_terrain should dispatch command', async () => {
      await registry.execute(
        'modify_terrain',
        'room-1',
        { actorId: 'a1', center: { x: 0, y: 0, z: 0 }, radius: 5, type: 'dig' },
        {}
      );
      expect(mockDispatch).toHaveBeenCalledWith(
        'room-1',
        expect.arrayContaining([expect.objectContaining({ type: 'MODIFY_TERRAIN' })])
      );
    });
    ...
    it('long_rest should dispatch command', async () => {
      ...
      expect(mockDispatch).toHaveBeenCalledWith(
        'room-1',
        expect.arrayContaining([expect.objectContaining({ type: 'LONG_REST' })])
      );
    });
    ...
    it('drop_item should dispatch command', async () => {
      ...
      expect(mockDispatch).toHaveBeenCalledWith(
        'room-1',
        expect.arrayContaining([expect.objectContaining({ type: 'DROP_ITEM' })])
      );
    });
    ...
    it('pickup_item should dispatch command', async () => {
      ...
      expect(mockDispatch).toHaveBeenCalledWith(
        'room-1',
        expect.arrayContaining([expect.objectContaining({ type: 'PICKUP_ITEM' })])
      );
    });
    ...
    it('throw_item should dispatch command', async () => {
      ...
      expect(mockDispatch).toHaveBeenCalledWith(
        'room-1',
        expect.arrayContaining([expect.objectContaining({ type: 'THROW_ITEM' })])
      );
    });
  });
  ...
  it('legacy wrapper should call main handler', async () => {
    ...
    await registry.execute(
      'perform_attack_legacy',
      'room-1',
      { attackerId: 'a1', targetId: 't1', actionName: 'hit' },
      {}
    );
    expect(mockDispatch).toHaveBeenCalledWith(
      'room-1',
      expect.arrayContaining([expect.objectContaining({ type: 'ATTACK' })])
    );
  });
