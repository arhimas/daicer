...
describe('Entity Lifecycle Service', () => {
    ...
    beforeEach(() => {
        ...
        mockDerive.mockReturnValue({
            level: 1, hp: 10, maxHp: 10, ac: 10, speed: { walk: 30 }, structuredActions: []
        });
    });
    ...
    describe('onboardPlayer', () => {
       it('should create entity and sheet', async () => {
           ...
           await service.onboardPlayer('r1', { name: 'Hero', race: 'Human', class: 'Fighter' }, { documentId: 'u1', id: 'u1', username: 'U1' });
       ...
       it('should handle avatar upload failure gracefully', async () => {
            ...
            const result = await service.onboardPlayer('r1', { 
                name: 'Player', 
       ...
       it('should handle missing race and class data gracefully', async () => {
            ...
            const result = await service.onboardPlayer('r1', { name: 'Player', race: 'NonExistent', class: 'NonExistent' }, { documentId: 'u1' });
       ...
       it('should throw error if player not in room', async () => {
            ...
            await expect(service.onboardPlayer('r1', { name: 'Player' }, { documentId: 'u1' }))
                .rejects.toThrow('User is not a player');
       });
