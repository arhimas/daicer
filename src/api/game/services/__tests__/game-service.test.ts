  ...
  service: vi.fn((uid) => {
    switch (uid) {
      case 'api::game.world-generation':
        return mockWorldGenService;
      case 'api::game.turn-processing':
        return mockTurnProcessingService;
      case 'api::game.entity-lifecycle':
        return mockEntityLifecycleService;
      case 'api::voxel-engine.voxel-engine':
        return mockVoxelEngineService;
    }
  }),
  documents: vi.fn((uid) => {
    switch (uid) {
...
describe('Game Service', () => {
  ...
  describe('startGame', () => {
    const mockRoom = {
      documentId: 'room1',
      players: [{ name: 'P1', isReady: true, user: { documentId: 'u1' }, characterSheet: { documentId: 's1' } }],
    ...
    it('should start successfully', async () => {
      ...
      expect(mockTurnRepo.create).toHaveBeenCalledWith(
