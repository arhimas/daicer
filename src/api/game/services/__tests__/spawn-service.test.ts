...
describe('Spawn Service', () => {
  ...
  describe('spawnMonster', () => {
    ...
    it('should fail on collision', async () => {
      ...
      await expect(service.spawnMonster('room-1', 'm1', { x: 0, y: 0, z: 0 })).rejects.toThrow('occupied');
    });
  ...
  describe('Hardening Edge Cases', () => {
    it('spawnMonster: should throw if room not found', async () => {
      ...
      await expect(service.spawnMonster('invalid-room', 'm1', { x: 0, y: 0, z: 0 })).rejects.toThrow('Room not found');
    });
    ...
    it('spawnMonster: should rethrow creation errors', async () => {
      ...
      await expect(service.spawnMonster('r1', 'm1', { x: 0, y: 0, z: 0 })).rejects.toThrow('DB Creation Failed');
    });

    it('spawnCharacter: should throw if blueprint not found', async () => {
      mockFindOne.mockResolvedValueOnce(null);
      await expect(service.spawnCharacter('r1', 'bad-c1', { x: 0, y: 0, z: 0 })).rejects.toThrow(
        'Character blueprint not found'
      );
    });
    ...
    it('spawnCharacter: should throw if room not found', async () => {
      ...
      await expect(service.spawnCharacter('bad-room', 'c1', { x: 0, y: 0, z: 0 })).rejects.toThrow('Room not found');
    });
    ...
    it('spawnCharacter: should fail on collision', async () => {
      ...
      await expect(service.spawnCharacter('r1', 'c1', { x: 0, y: 0, z: 0 })).rejects.toThrow('occupied');
    });
