...
describe('Turn Pipeline Service (Comprehensive)', () => {
          ...
          events: [
            {
              ...
              actor: 'a1',
              timestamp: 123,
              payload: { from: { x: 0, y: 0, z: 0 }, to: { x: 1, y: 0, z: 0 }, mode: 'walk' },
            } as GameEvent,
          ],
          stateDiff: {
  ...
  describe('Phase 1: Intent & Parsing', () => {
    ...
    it('should ignore invalid MOVE format (non-numeric)', async () => {
      ...
      expect(mockStrapi.log.warn).toHaveBeenCalledWith(
        expect.stringContaining('Unknown text input'),
        expect.anything()
      );
    });
  ...
  describe('Phase 2: Guardian (Locking)', () => {
    ...
    it('should release lock even if processing fails', async () => {
      ...
      await expect(
        turnPipeline.processTurn('room-1', [{ type: 'text', text: 'MOVE:1,1', agentId: 'a1' }])
  ...
  describe('Phase 5: Narration & Snapshot', () => {
    ...
    it('should save narrative summary to turn and create message', async () => {
      ...
      // Create Message
      expect(mockDocCreaters['api::message.message']).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('processRoomTurn Helper', () => {
    it('should parse JSON actions from players', async () => {
      mockDocFindOne.mockResolvedValueOnce({
        ...
        players: [
          {
            action: JSON.stringify({ type: 'ATTACK', payload: { actorId: 'p1', targetId: 'e1' } }),
            character: { documentId: 'p1' },
          },
        ],
      });
    ...
    it('should clear player actions after success', async () => {
      mockDocFindOne.mockResolvedValueOnce({
        documentId: 'room-1',
        players: [{ action: 'MOVE:1,1', character: { documentId: 'p1' } }],
      });
