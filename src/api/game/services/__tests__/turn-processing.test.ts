...
describe('Turn Processing Service', () => {
  describe('executeDeterministicTurn', () => {
    it('processes valid moves', async () => {
      const mockRoom = {
        documentId: 'room-1',
        entity_sheets: [{ documentId: 'hero-1', position: { x: 0, y: 0, z: 0 }, name: 'Hero' }],
        exploredTiles: [],
      };
      ...
      strapi.service.mockImplementation((name) => {
        if (name === 'api::game.turn-persistence')
          return {
      ...
      await service.executeDeterministicTurn('room-1', [{ type: 'move', entityId: 'hero-1', payload: { x: 1, y: 1 } }]);
    ...
    it('prevents collision', async () => {
      ...
      strapi.service.mockImplementation((name) => {
        if (name === 'api::game.turn-persistence')
          return { updateCharacterPosition: updatePosMock, persistTurn: persistTurnMock };
        return {};
      });
