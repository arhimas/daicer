...
vi.mock('../../src/engine/entropy', () => {
  return {
    EntropySystem: class {
      ...
      advanceTurn(...args: any[]) {
        return mockAdvanceTurn(...args);
      }
    },
  };
});
...
const mockStrapi: any = {
  service: vi.fn((uid) => {
    switch (uid) {
      ...
      case 'api::game.game-ledger':
        return mockLedger;
      case 'api::game.spawn-service':
        return mockSpawnService;
      case 'api::game.biome-spawn-service':
        return mockBiomeSpawnService;
...
describe('Turn Processing', () => {
  ...
  describe('executeDeterministicTurn', () => {
    it('should dispatch attack actions', async () => {
      ...
      expect(mockActionEngine.dispatch).toHaveBeenCalledWith(
        'r1',
        expect.arrayContaining([
          expect.objectContaining({
            type: 'ATTACK',
            payload: expect.objectContaining({ actorId: 'u1', targetId: 't1' }),
          }),
        ])
      );
    });
    ...
    it('should handle exploration updates (new tiles)', async () => {
      ...
      await service.executeDeterministicTurn('r1', [{ type: 'move', entityId: 'p1', payload: { x: 1, y: 1 } }]);
    ...
    it('should trigger chunk spawn on new chunk discovery', async () => {
      ...
      await service.executeDeterministicTurn('r1', [{ type: 'move', entityId: 'p1', payload: { x: 64, y: 64 } }]);
  ...
  describe('Hardening Edge Cases', () => {
    it('executeDeterministicTurn: should block move on collision', async () => {
      ...
      await service.executeDeterministicTurn('r1', [{ type: 'move', entityId: 'p1', payload: { x: 10, y: 10 } }]);
    ...
    it('executeDeterministicTurn: should handle spawn errors gracefully', async () => {
      ...
      // Should not throw
      await service.executeDeterministicTurn('r1', [{ type: 'spawn', payload: { entityType: 'monster', id: 'm1' } }]);
    ...
    it('executeDeterministicTurn: should handle attack dispatch errors gracefully', async () => {
      ...
      expect(mockLogError).toHaveBeenCalledWith(
        expect.stringContaining('Failed to dispatch attack'),
        expect.any(Error)
      );
    });
    ...
    it('executeDeterministicTurn: should handle exploration update failure', async () => {
      ...
      await service.executeDeterministicTurn('r1', [{ type: 'move', entityId: 'p1', payload: { x: 1, y: 1 } }]);
      expect(mockLogError).toHaveBeenCalledWith(
        expect.stringContaining('Exploration update failed'),
        expect.any(Error)
      );
    });
