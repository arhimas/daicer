      ...
      switch (attr.type) {
        ...
        case 'enumeration':
          if (!attr.enum.includes(value)) {
            errors.push(
              `Invalid enum value for ${fieldPath}: got '${value}', expected one of [${attr.enum.join(', ')}]`
            );
          }
          break;
        case 'component':
        ...
        case 'dynamiczone':
          ...
          for (let i = 0; i < value.length; i++) {
            ...
            try {
              ...
              errors.push(
                `Failed to load schema for dynamic zone component '${componentData.__component}' at ${fieldPath}[${i}]`
              );
            }
          }
          break;
