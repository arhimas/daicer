...
export const CastingConfigSchema = z
  .object({
    time_value: z.number().int().min(1).default(1),
    time_unit: z
      .enum(['Action', 'Bonus Action', 'Reaction', 'Minute', 'Hour', 'Day', 'Round', 'Year'])
      .default('Action'),
...
export const RangeConfigSchema = z
  .object({
...
export const MechanicsConfigSchema = z
  .object({
    action_type: z
      .enum([
        'Melee Spell Attack',
...
export const DamageInstanceSchema = z
  .object({
...
export const ConditionInstanceSchema = z
  .object({
...
export const EquipmentDataSchema = z
  .object({
