...
const mapType = (attr: any, componentMap: Record<string, string>): string => {
  if (attr.type === 'string' || attr.type === 'text' || attr.type === 'uid' || attr.type === 'richtext') return 'z.string()';
...
const generate = () => {
    ...
    components.forEach(({ schemaName, json }) => {
        ...
        for (const [key, attr] of Object.entries(attributes)) {
             const zodType = mapType(attr, componentMap);
