...
export const TextureInput = React.forwardRef<HTMLInputElement, TextureInputProps>((props, _ref) => {
  ...
  useEffect(() => {
    ...
    const defaultLayers = Array(MAX_Z + 1)
      .fill(null)
      .map(() =>
        Array(GRID_SIZE)
          .fill(null)
    ...
    if (value) {
      try {
        ...
        if (Array.isArray(parsedValue) && parsedValue.length > 0) {
          const emptyLayers = Array(MAX_Z + 1)
            .fill(null)
            .map(() =>
              Array(GRID_SIZE)
                .fill(null)
          ...
          parsedValue.forEach(
            (v: { x: number; y: number; z: number; type?: string; block?: string }) => {
              if (v.z === 0 && v.y >= 0 && v.y < GRID_SIZE && v.x >= 0 && v.x < GRID_SIZE) {
                if (!loadedTiles[0])
                  loadedTiles[0] = Array(GRID_SIZE)
                    .fill(null)
                    .map(() => Array(GRID_SIZE).fill(null));
    ...
    if (requiresAutoFill && !value) {
      ...
      setTimeout(() => {
        onChange({
          target: { name, value: JSON.stringify(flattenedVoxels), type: attribute.type },
        });
      }, 100);
    }
  }, [value]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (chunk && chunk.tiles && chunk.tiles[0]) {
        inputPixels = chunk.tiles[0].map((row) =>
          row.map((cell) => (cell ? cell.block : 'transparent'))
        );
      }
        ...
        <Flex direction="column" gap={4}>
          ...
          <Flex justifyContent="space-between" alignItems="center" wrap="wrap" gap={2}>
            <Flex gap={2}>
              ...
              <Button variant="danger-light" onClick={handleClear} startIcon={<Trash />} size="S">
                Clear
              </Button>
            </Flex>
          ...
          <Box>
            ...
            <Flex gap={4} paddingTop={4} alignItems="center" wrap="wrap">
              ...
              <Button variant="tertiary" onClick={handleOpenData} startIcon={<Code />} size="S">
                Data
              </Button>
            </Flex>
          </Box>
