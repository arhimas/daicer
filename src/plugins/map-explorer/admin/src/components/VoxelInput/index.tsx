...
export const VoxelInput = React.forwardRef<HTMLInputElement, VoxelInputProps>((props, _ref) => {
    ...
    const fetchTerrains = async () => {
      try {
        const { data } = await get(
          '/content-manager/collection-types/api::terrain.terrain?page=1&pageSize=100&populate=*'
        );
  ...
  useEffect(() => {
    ...
    if (isWorldContext) {
      ...
      const loadWorldChunk = async () => {
        try {
          const { data: worldData } = await get(
            `/content-manager/collection-types/api::world.world/${worldId}`
          );
          if (worldData) {
            const payload = {
              x: 0,
    ...
    if (value) {
      try {
        ...
        if (Array.isArray(parsedValue)) {
          parsedValue.forEach(
            (v: { x: number; y: number; z: number; block?: string; type?: string }) => {
              if (
                v.z >= 0 &&
                v.z <= MAX_Z &&
                v.y >= 0 &&
                ...
                if (!loadedTiles[v.z])
                  loadedTiles[v.z] = Array(gridSize)
                    .fill(null)
                    .map(() => Array(gridSize).fill(null));
  ...
  useEffect(() => {
    ...
    RenderEngine.render(ctx, chunk, currentZ, scale, pan, terrains, {
      showGrid: true,
      ...
      {isModalOpen && (
        ...
        >
          ...
          >
            ...
            <Box paddingTop={4}>
              <SingleSelect label="Model" value={model} onChange={setModel}>
