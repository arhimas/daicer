...
export const WorldVoxelInput = React.forwardRef<HTMLInputElement, WorldVoxelInputProps>(
  (props, _ref) => {
    ...
    */
      ...
      const fetchTerrains = async () => {
        try {
          const { data } = await get(
            '/content-manager/collection-types/api::terrain.terrain?page=1&pageSize=100'
          );
    ...
    const handleBrushAction = async (e: React.MouseEvent) => {
      ...
      newChunk.tiles[currentZ][tileY][tileX] = {
        x: tileX,
        y: tileY,
        z: currentZ, // Local or Global? Chunk stores local x/y typically
        block: selectedBlock,
        biome: 'custom',
        isWalkable: true,
        isTransparent: false,
        ...
        <Box background="neutral100" padding={4} hasRadius>
          <Flex direction="column" alignItems="center" gap={2}>
            ...
            <Typography variant="omega">
              Please save the World to enable the Voxel Editor.
            </Typography>
          </Flex>
        </Box>
      );
