...
const controller = ({ strapi }) => ({
    ...
    async getMapChunk(ctx) {
        ...
        try {
            ...
            const chunk = await strapi.service('api::voxel-engine.voxel-engine').getChunk(typeof x === 'string' ? parseInt(x) : 0, typeof y === 'string' ? parseInt(y) : 0, config, worldId);
            ctx.body = chunk;
        }
        catch (err) {
    ...
    async updateVoxel(ctx) {
        ...
        try {
            await strapi.service('api::voxel-engine.voxel-engine').editVoxel(chunkX, chunkY, voxelX, voxelY, voxelZ, newType, reason, worldId);
    ...
    async generateTexture(ctx) {
        ...
        const GRID_SIZE = 32;
        const tiles = Array(GRID_SIZE).fill(null).map(() => Array(GRID_SIZE).fill(null).map(() => null));
    ...
    async getTimeline(ctx) {
        ...
        if (!roomId)
            return ctx.badRequest('Missing roomId');
        try {
            const data = await strapi.service('api::game.history-service').getTimelineData(roomId);
            ctx.body = data;
        }
        catch (err) {
