    ...
    describe('getDeepPopulate', () => {
        ...
        it('should build nested populate for relations', () => {
            ...
            mockGetModel.mockImplementation((uid) => {
                if (uid === 'api::a.a') return { attributes: { b: { type: 'relation', target: 'api::b.b' } } };
                if (uid === 'api::b.b') return { attributes: { c: { type: 'relation', target: 'api::c.c' } } };
        ...
        it('should handle circular dependencies (A -> B -> A) by stopping recursion', () => {
            mockGetModel.mockImplementation((uid) => {
                if (uid === 'api::cycle-a.cycle-a') return { attributes: { b: { type: 'relation', target: 'api::cycle-b.cycle-b' } } };
                if (uid === 'api::cycle-b.cycle-b') return { attributes: { a: { type: 'relation', target: 'api::cycle-a.cycle-a' } } };
                return { attributes: {} };
            });
