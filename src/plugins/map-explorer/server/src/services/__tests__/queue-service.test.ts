...
describe('Queue Service', () => {
  ...
  describe('Job Management', () => {
     ...
     it('should add pixel job', async () => {
         ...
         expect(sharedMocks.add).toHaveBeenCalledWith('generate-sprite', expect.objectContaining({ size: 'medium' }), expect.any(Object));
     });

     it('should add blueprint job', async () => {
         await service.addBlueprintJob({ type: 'blueprint' });
         expect(sharedMocks.add).toHaveBeenCalledWith('generate-blueprint', expect.anything(), expect.anything());
     });
     ...
     it('should route addJob correctly', async () => {
         ...
         expect(sharedMocks.add).toHaveBeenCalledWith('generate-sprite', expect.anything(), expect.anything());
     });
  });
  ...
  describe('Accessors', () => {
     ...
     it('should fall back to blueprint queue', async () => {
         sharedMocks.getJob.mockResolvedValueOnce(null).mockResolvedValueOnce({ id: 'j1', name: 'bp' });
         const job = await service.getJob('j1');
     ...
     it('should return summary', async () => {
         sharedMocks.getJobCounts.mockResolvedValue({ active: 5, completed: 0, failed: 0, waiting: 0 });
  ...
  describe('Worker Logic', () => {
      ...
      it('should handle pixel worker errors', async () => {
          ...
          expect(mockLogError).toHaveBeenCalledWith(expect.stringContaining('Job j1 Failed'), expect.anything());
      });
      ...
      it('should handle blueprint worker errors', async () => {
          ...
          expect(mockLogError).toHaveBeenCalledWith(expect.stringContaining('Job b1 Failed'), expect.anything());
      });
  });
  ...
  describe('Uninitialized State', () => {
      ...
      it('should throw when adding job if not initialized', async () => {
          ...
          await expect(freshService.addBlueprintJob({})).rejects.toThrow('Blueprint Queue not initialized');
      });
  });
});
