...
describe('DashboardController', () => {
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle single queue failure in getStats (inner try/catch)', async () => {
             ...
             const q2 = { ...mockQueue, name: 'bad-queue', getJobCounts: vi.fn().mockRejectedValue(new Error('Boom')) };
             ...
             const customStrapi = {
                 ...mockStrapi,
                 plugin: vi.fn((uid) => {
                     if (uid === 'queue-dashboard') return { config: vi.fn(() => ['good-queue', 'bad-queue']) };
                     if (uid === 'bullmq') return { 
                         service: vi.fn(() => ({ 
                             get: vi.fn((name) => {
        ...
        it('should handle top-level error in getStats (outer try/catch)', async () => {
             ...
             const brokenStrapi = {
                 plugin: vi.fn(() => { throw new Error('Plugin System Broken'); })
             } as any;
