      ...
      for (const entry of data) {
        ...
        if (entry.actions && Array.isArray(entry.actions)) {
          for (const act of entry.actions) {
            ...
            const cleanName = act.name
              .toLowerCase()
              .replace(/\s*\([^)]*\)/g, '')
              .trim(); // Remove (Two-Handed) etc
            ...
            if (libraryItem && act.type.toLowerCase().includes('weapon')) {
              ...
              if (monsterDamage && itemDamage && monsterDamage !== itemDamage) {
                ...
                if (!dbItemMap.has(variantSlug)) {
                  ...
                  try {
                    ...
                    console.warn(
                      `      ⚠️ Failed to create variant ${variantSlug}, falling back to Action. ${e.message}`
                    );
                    useItem = false;
                  }
                }
              ...
              if (useItem) {
                ...
                if (itemId) {
                  ...
                  const exists = inventoryComponent.find(
                    (i) => i.item === itemId || dbItemMap.get(i.item as string) === itemId
                  );
                  if (!exists) {
